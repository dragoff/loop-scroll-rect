...
namespace UnityEngine.UI
{
        ...
        IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        ...
        [SerializeField]
        private MovementType movement = MovementType.Elastic;
        ...
        protected override void Awake()
        {
            ...
            if (Application.isPlaying)
            {
                ...
                Debug.Assert(Mathf.Abs(GetDimension(content.pivot)) == value, "Check Content pivot Y. Must be 0 or 1.");
                Debug.Assert(
                    Mathf.Abs(GetDimension(content.anchorMin)) == value ||
                    Mathf.Abs(GetDimension(content.anchorMax)) == value,
                    "Check Content anchor. Must be Top-Center or Bottom-Center");
            }
        }
#endif
        ...
        public void RefreshCells()
        {
            if (Application.isPlaying && this.isActiveAndEnabled)
            {
                ...
                for (int i = 0; i < content.childCount; i++)
                {
                    ...
                    {
                        dataSource.ProvideData(content.GetComponent<PoolObject>(), itemTypeEnd);
                        itemTypeEnd++;
                    }
                    else
                    {
        ...
        public void RefillCells(int offset = 0, bool fillViewRect = false)
        {
            ...
            if (fillViewRect && itemSize > 0 && sizeFilled < sizeToFill)
            {
                ...
                if (newOffset != offset)
                    RefillCells(
                        newOffset); //refill again, with the new offset value, and now with fillViewRect disabled.
            }
        ...
        protected RectTransform GetFromTempPool(int itemIdx)
        {
            ...
            dataSource.ProvideData(nextItem.GetComponent<PoolObject>(), itemIdx);
            return nextItem;
        }
        ...
        public virtual void OnDrag(PointerEventData eventData)
        {
            ...
            position += offset;
            if (movement == MovementType.Elastic)
            {
                //==========LoopScrollRect==========
                if (offset.x != 0)
        ...
        protected virtual void LateUpdate()
        {
            ...
            if (!m_Dragging && (offset != Vector2.zero || m_Velocity != Vector2.zero))
            {
                ...
                for (int axis = 0; axis < 2; axis++)
                {
                    ...
                    if (movement == MovementType.Elastic && offset[axis] != 0)
                    {
        ...
        private Vector2 CalculateOffset(Vector2 delta)
        {
            ...
            if (movement == MovementType.Unrestricted)
                return offset;
            if (movement == MovementType.Clamped)
            {
                if (TotalCount < 0)
